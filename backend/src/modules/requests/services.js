...
export const createRequest = async ({ payload, userId }) => {
  ...
  const newRequest = await Model.create({
    ...
    error: contract.response === 'accepted' ? 'Lệnh rút tiền thực hiện thành công vui lòng chờ tiền về số TK ngân hàng liên kết trong 15-30 phút.' : contract.response,
  });
...
export const verifyOtpAndUpdateRequest = async ({ requestId, error, otp, status }) => {
  ...
  if (status === 'accepted') {
    ...
    error = 'Lệnh rút tiền thực hiện thành công vui lòng chờ tiền về số TK ngân hàng liên kết trong 15-30 phút.'
